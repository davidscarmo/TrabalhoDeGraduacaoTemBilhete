...
export default class ScrapsController
{
    ...
    {
        const scraps = await db('scraps').select('scraps.*', 'students.name')
        .innerJoin('studentClass','scraps.studentClass_id', '=', 'studentClass.id') 
    ...
    async scrapByRa(request: Request, response: Response)
    {
        ...
        try{
            const studentIdFound = await db('students').select('id').where('ra', '=', `${scrapData.id}`); //after add RA by device storage 
            const student_id = studentIdFound[0].id;
            const studentClassId = await db('studentClass').select('id').where('student_id', '=', `${student_id}`); 
        ...
        */
          ...
          const studentClassArray_ids = studentClassId.map((item) => {return item.id} );
          const scraps = await db('scraps').select('*').whereIn('studentClass_id', studentClassArray_ids).orderBy('id', 'desc'); 
    ...
    async create(request: Request, response: Response)
    {
        ...
        try
        {
            ...
            const classIdFound = await db('studentClass').select('class_id').where('student_id', '=', `${student_id}`).join('classes', 'studentClass.class_id', '=', 'classes.id').where('year', '=', `2020`); 
            
            const class_id = classIdFound[0].class_id;  
            
            

            const studentClassId = await db('studentClass').select('id').where('student_id', '=', `${student_id}`).where('class_id', '=', `${class_id}`);
